...
>(
  ...
  const TableWithBatteries = useDeepCompareMemo(
    () =>
      React.forwardRef(({ children, ...restProps }: Omit<TableProps, 'ref'>, ref: React.Ref<HTMLTableElement>) => (
        <Table {...propHelpers.tableProps} innerRef={ref as React.MutableRefObject<HTMLTableElement>} {...restProps}>
          {children}
        </Table>
      )),
